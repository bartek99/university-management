...
export default function AddScheduleItemForm(props: AddScheduleItemFormProps) {
    ...
    const scheduleItemId = params.scheduleItemId;
    ...
    useEffect(() => {
        let mounted = true;
        scheduleItemApi.getScheduleItemById(scheduleItemId)
            .then((scheduleItem) => {
                if (mounted && scheduleItem) {
                    reset(scheduleItemApi.editedScheduleItem(scheduleItem));
                }
            })
        return () => {
            mounted = false;
        }
    }, [reset, scheduleItemId]);

    const onSubmit = (scheduleItem: scheduleItemApi.EditedScheduleItem) => {
        props.setResult(null);
        scheduleItemApi.updateScheduleItem(scheduleItemId, scheduleItem)
            .then((result) => {
                if (result === null) {
                    reset(scheduleItem);
