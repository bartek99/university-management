...
export default function EditStudentForm(props: EditStudentFormProps) {
    ...
    useEffect(() => {
        let mounted = true;
        user.getStudent(props.userId)
            .then((student) => {
                if (mounted && student) {
                    reset(user.editedStudent(student));
                }
            })
        return () => {
            mounted = false;
    ...
    const onSubmit = (editedStudent: user.EditedStudent) => {
        props.setResult(null);
        user.userExists(editedStudent.user.email, props.userId)
            .then((result) => {
                if (result === true) {
                    setError('user.email', { message: 'Użytkownik o tym adresie e-mail już istnieje' })
                } else {
                    user.updateStudent(props.userId, editedStudent)
                        .then((student) => {
                            if (student) {
                                reset(editedStudent);
                    ...
                    <Controller
                        ...
                        as={
                            <TextField
                                ...
                                error={errors.albumNumber ? true : false}
                                helperText={errors.albumNumber && errors.albumNumber.message}
                            />
                        }
                        control={control}
                        defaultValue=""
